...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const updateActiveSection = () => {
      ...
      for (const link of sectionLinks) {
        ...
        if (rect.top <= 140 && rect.bottom > 140) { currentSection = link.label; }
      }
  ...
  const handleNavClick = (e, link) => {
    if (!link.sectionId) { setMobileOpen(false); return; }
    e.preventDefault();
    setMobileOpen(false);
  ...
  const getLinkClassName = (label) => {
    ...
    return `font-body tracking-[0.2em] uppercase transition-all duration-300 text-xs px-3 py-1.5 ${
      isActive 
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 transition-all duration-500 bg-background/95 backdrop-blur-md border-b border-border">
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between h-20">
          <a href="/" className="flex items-center h-10">
            <img src="/logos/bmills-logo.png" alt="B.Mills" className="h-10 w-auto object-contain" />
          </a>

          <div className="hidden md:flex items-center gap-6">
            {navLinks.map((link) => (
              <a key={link.href} href={link.href} onClick={(e) => handleNavClick(e, link)} className={getLinkClassName(link.label)}>
                {link.label}
              </a>
            ))}
            ...
            <a href={inquiryLink.href} onClick={(e) => handleNavClick(e, inquiryLink)}
              className="font-body text-xs tracking-[0.2em] uppercase bg-foreground text-background px-5 py-2.5 hover:bg-foreground/80 transition-colors duration-300 ml-2">
              {inquiryLink.label}
            </a>
          </div>
        ...
        {mobileOpen && (
          <motion.div className="fixed inset-0 z-[100] bg-background flex flex-col items-center justify-center"
            ...
            <div className="flex flex-col items-center gap-8">
              ...
              {navLinks.map((link, i) => (
                <motion.a key={link.href} href={link.href} onClick={(e) => handleNavClick(e, link)}
